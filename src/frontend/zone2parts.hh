...
public:
    ZoneState(uint32_t zone_id, uint32_t region_id, uint32_t total_nodes_num, uint32_t recovery_time, uint32_t to_vul_num){
        zone_id_ = zone_id;
  ...
  /*
   * @brief: judge if there is any preemption occured in this zone
   */
  bool ifPreempt() {
    return !(unavailable_nodes_id.empty() && new_unavailable_nodes_id.empty());
  }

  int preemptNum() {
    return new_unavailable_nodes_id.size();
  }
   ...
   */
   ...
   auto FlagInitialization(int num) -> void {
        if (available_nodes_id.empty()) {
            return;
        }
        if (num > available_nodes_id.size()) {
            FlagInitialization();
            return;
        }
        for (int i = 0; i < num; i++) {
            auto start_node_id = available_nodes_id.at(i);
            assert(nodes_[start_node_id].state_ == NodeFlag::INVULNERABLE);
            nodes_[start_node_id].state_ = NodeFlag::VULNERABLE;
            invulnerable_nodes_id.erase(start_node_id);
            vulnerable_nodes_id.insert(start_node_id);
    ...
    auto get_region_id() const -> uint32_t {return region_id_;}
    auto get_zone_id() const -> uint32_t {return zone_id_;}
    auto get_total_nodes_num() const -> uint32_t {return total_nodes_num_;}
